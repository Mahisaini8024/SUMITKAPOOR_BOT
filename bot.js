...
async function verifyUIDViaYubitAPI(uid) {
  ...
  try {
    ...
    if (valJson.code !== 0 && valJson.code !== '0') {
      return { isReferral: false, depositOk: false, deposit: '0', error: valJson.message || valJson.msg || 'YUBIT API error' };
    }

    const isReferral = valJson.data === true;
    if (!isReferral) {
      return { isReferral: false, depositOk: false, deposit: '0', error: null };
    }
